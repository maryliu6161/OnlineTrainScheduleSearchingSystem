...
public class AddData extends JFrame implements ActionListener
{
	...
	public void initCardPanel()
	{
		...
		jbTsub.setBounds(250,300,60,30);
		jpTrain.add(jbTsub);
		jbTsub.addActionListener(this);
		//设置确定按钮的位置及大小并注册事件监听器
		jbSsub.setBounds(250,300,60,30);
		jpStation.add(jbSsub);
		jbSsub.addActionListener(this);
		//设置确定按钮的位置及大小并注册事件监听器
		jbRsub.setBounds(250,300,60,30);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource()==jbTrain)
		{//按下车次添加按钮
			//显示卡片train
			cl.show(jpRight,"train");
		}
		else if(e.getSource()==jbStation)
		{//按下车站添加按钮
			cl.show(jpRight,"station");
		}
		else if(e.getSource()==jbRelation)
		{//按下关系添加按钮
		...
		else if(e.getSource()==jbSsub)
		{//按下车站提交按钮
			submitStation();
		}
		else if(e.getSource()==jbRsub)
		{//按下关系提交按钮
			submitRelation();
		}
	}
