						...
						ActionListener,MouseListener,ListSelectionListener,KeyListener
{
	...
	{
		//设置复选框的大小
		jcbReverseStation.setBounds(400,50,130,30);
		//将复选框加到JPanel中
		jp.add(jcbReverseStation);
		//注册事件监听器
		jcbReverseStation.addActionListener(this);
		jcbMidStation.setBounds(550,50,80,30);
...
//		jcbStart.addActionListener(this);
	...
	public void searchStation()
	{
		if(jrbArray[0].isSelected())
		{//站站查询
			if(jcbReverseStation.isSelected()&&jcbMidStation.isSelected())
			{//当两个复选框都选中时
				this.zzcxAndMid();
			}
			else if(jcbReverseStation.isSelected()&&!jcbMidStation.isSelected())
			{//当反向车站选中,无中转站时
				this.zzcx();
			}
			else if(!jcbReverseStation.isSelected()&&jcbMidStation.isSelected())
	...
	public Vector<Vector> combine(Vector<Vector> temp,
							Vector<Vector> temp1,Vector<Vector> temp2)
	{//将这三个Vector组合成一个	
